...
from ai.query_processor.models import IntentType, EntityType, FilterType, TechnologyType
...
class TestSpellingCorrector:
    ...
    def test_correct_unknown_word_ignored(self):
        corrector = SpellingCorrector(max_distance=2)
        known = {"python"}
    ...
    def test_correct_short_word_unchanged(self):
        corrector = SpellingCorrector()
        known = {"go", "rust"}
...
class TestSynonymResolver:
    def test_resolve_ml_to_machine_learning(self):
...
class TestIntentExtraction:
    ...
    def test_spelling_mistake_still_detects_search(self, engine):
...
class TestTechnologyExtraction:
    ...
    def test_detects_django(self, engine):
        result = engine.understand("django rest api")
        assert "Django" in result.frameworks

    def test_detects_postgresql(self, engine):
        result = engine.understand("postgresql database tool")
        assert "PostgreSQL" in result.databases or "PostgreSQL" in [t.name for t in result.technologies]

    def test_detects_docker(self, engine):
    ...
    def test_multiple_technologies(self, engine):
    ...
    def test_detects_tensorflow(self, engine):
        result = engine.understand("tensorflow deep learning")
        assert "TensorFlow" in result.frameworks or "TensorFlow" in [t.name for t in result.technologies]
...
class TestFilterExtraction:
    ...
    def test_cross_platform_filter(self, engine):
...
class TestFullPipeline:
    ...
    def test_whitespace_query(self, engine):
        result = engine.understand("   ")
        assert result.confidence == 0.0
    ...
    def test_no_crash_on_special_chars(self, engine):
        result = engine.understand("###  $$$  @@@")
        assert result.confidence == 0.0
...
class TestLLMExtractor:
    def test_llm_extractor_disabled_by_default(self, engine):
        assert engine._llm_extractor is None

    def test_llm_extractor_graceful_degradation(self):
