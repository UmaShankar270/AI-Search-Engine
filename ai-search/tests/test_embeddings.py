...
class TestCompositionStrategy:
    ...
    def test_for_repository_with_readme(self):
        result = CompositionStrategy.for_repository(
            name="test",
            description="desc",
    ...
    def test_for_repository_empty(self):
        result = CompositionStrategy.for_repository()
        assert result == ""

    def test_for_metadata_empty(self):
        assert CompositionStrategy.for_metadata("") == ""

    def test_for_metadata_normalizes(self):
...
class TestEmbeddingGeneratorReal:
    ...
    def test_generate_for_repository_partial(self, generator):
        vec = generator.generate_for_repository(name="test-repo")
        assert vec.shape == (generator.dim,)

    def test_cache_hit_returns_same_vector(self, generator):
        vec1 = generator.generate("cache test string")
        vec2 = generator.generate("cache test string")
        assert np.allclose(vec1, vec2)

    def test_cache_miss_different_returns_different(self, generator):
    ...
    def test_dim_property(self, generator):
        dim = generator.dim
        assert dim == 384 or dim > 0

    def test_cache_stats_property(self, generator):
    ...
    def test_generate_for_metadata(self, generator):
        vec = generator.generate_for_metadata("name owner stars 100")
        assert vec.shape == (generator.dim,)

    def test_similar_query_embeddings(self, generator):
...
class TestEmbeddingGeneratorUnit:
    def test_with_mock_model(self, mocker):
    ...
    def test_batch_with_mock(self, mocker):
        mock_mgr = mocker.patch("ai.embeddings.generator.ModelManager")
        mock_mgr.return_value.dim = 384
        mock_mgr.return_value.encode.return_value = np.array([[0.1] * 384, [0.2] * 384], dtype=np.float32)
