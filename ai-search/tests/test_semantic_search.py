...
@pytest.fixture
def sample_repos(sample_ids, sample_metadata) -> list[dict]:
    return [
...
class TestFAISSVectorIndex:
    ...
    def test_build_returns_stats(self, faiss_index):
    ...
    def test_size_after_build(self, built_index):
        assert built_index.size == 20

    def test_dim_returned(self, built_index):
        assert built_index.dim == DIM

    def test_is_trained_true_after_build(self, built_index):
        assert built_index.is_trained is True

    def test_search_returns_correct_count(self, built_index):
    ...
    def test_load_raises_on_missing_file(self, faiss_index):
        with pytest.raises(FileNotFoundError):
            faiss_index.load("/nonexistent/path.index")

    def test_save_raises_on_empty_index(self, faiss_index):
        with pytest.raises(RuntimeError):
            faiss_index.save("/tmp/should-not-exist.index")

    def test_clear_empties_index(self, built_index):
        built_index.clear()
        assert built_index.size == 0
        assert built_index.is_trained is False

    def test_rebuild_replaces(self, built_index):
        new_vecs = RNG.random((3, DIM)).astype(np.float32)
    ...
    def test_search_score_range_cosine(self, built_index):
        q = RNG.random(DIM).astype(np.float32)
        for h in built_index.search(q, top_k=5):
            assert 0.0 <= h.score <= 1.0

    def test_search_after_remove_add(self, built_index):
        built_index.remove(["repo-0", "repo-1"])
        built_index.add(RNG.random((2, DIM)).astype(np.float32), ["new-0", "new-1"])
        assert built_index.size == 20

    def test_search_returns_hash_ids(self, built_index):
        q = RNG.random(DIM).astype(np.float32)
        for h in built_index.search(q, top_k=5):
            int(h.repo_id)
...
class TestIndexMetadataStore:
    def test_add_and_retrieve(self, metadata_store):
    ...
    def test_repo_exists(self, metadata_store):
    ...
    def test_remove_mapping(self, metadata_store):
    ...
    def test_remove_nonexistent(self, metadata_store):
        assert metadata_store.remove("ghost") is None

    def test_clear(self, metadata_store):
        for i in range(10):
            metadata_store.add_mapping(i, f"repo-{i}")
        metadata_store.clear()
        assert metadata_store.size() == 0

    def test_items(self, metadata_store):
    ...
    def test_empty_store(self, metadata_store):
        assert metadata_store.size() == 0
        assert metadata_store.items() == []

    def test_add_without_metadata(self, metadata_store):
        metadata_store.add_mapping(5, "no-meta")
        assert metadata_store.get_metadata("no-meta") is None

    def test_metadata_isolation(self, metadata_store):
...
class TestSemanticSearchEngine:
    ...
    def test_search_custom_top_k(self, engine):
        assert len(engine.search("test", top_k=3).results) <= 3

    def test_search_results_have_metadata(self, engine):
    ...
    def test_search_by_vector_threshold(self, engine):
    ...
    def test_batch_search(self, engine):
        results = engine.batch_search(["q1", "q2"], top_k=3)
        assert len(results) == 2
        assert all(isinstance(r, SearchResult) for r in results)

    def test_batch_search_empty(self, engine):
        assert engine.batch_search([]) == []

    def test_build_index_from_repos(self, faiss_index, metadata_store):
        engine = SemanticSearchEngine(faiss_index, metadata_store, encoder=dummy_encoder, batch_encoder=dummy_batch_encoder)
        repos = [
    ...
    def test_build_index_empty_repos(self, faiss_index, metadata_store):
        engine = SemanticSearchEngine(faiss_index, metadata_store, encoder=dummy_encoder)
        stats = engine.build_index([])
        assert stats.total_vectors == 0

    def test_build_index_from_embeddings(self, faiss_index, metadata_store):
        engine = SemanticSearchEngine(faiss_index, metadata_store)
        vecs = RNG.random((3, DIM)).astype(np.float32)
        stats = engine.build_index_from_embeddings(vecs, ["e1", "e2", "e3"], [{"n": "A"}, {"n": "B"}, {"n": "C"}])
    ...
    def test_add_repositories(self, engine):
        engine.add_repositories([{"repo_id": "new-repo", "text": "new", "metadata": {"name": "New"}}])
        assert engine._metadata_store.repo_exists("new-repo")

    def test_add_embeddings(self, engine):
    ...
    def test_remove_repository(self, engine):
...
class TestIntegration:
    def test_full_pipeline(self):
        ...
        store = IndexMetadataStore()
        engine = SemanticSearchEngine(index, store, encoder=dummy_encoder, batch_encoder=dummy_batch_encoder)
        repos = [
            {"repo_id": "r1", "text": "gpu machine learning framework", "metadata": {"name": "TensorFlow"}},
        ...
        engine.add_repositories([{"repo_id": "r4", "text": "image processing", "metadata": {"name": "OpenCV"}}])
    ...
    def test_exact_match_boost(self):
        index = FAISSVectorIndex(dimension=DIM)
        store = IndexMetadataStore()
        engine = SemanticSearchEngine(index, store, encoder=dummy_encoder, boost_factor=2.0)
        repos = [
            {"repo_id": "pytorch", "text": "deep learning framework", "metadata": {"name": "PyTorch"}},
            {"repo_id": "tensorflow", "text": "machine learning platform", "metadata": {"name": "TensorFlow"}},
    ...
    def test_large_collection(self):
        ...
        repos = [{"repo_id": f"r-{i}", "text": f"content {i}", "metadata": {"name": f"Repo {i}"}} for i in range(200)]
        engine.build_index(repos)
        assert engine._vector_index.size == 200
        result = engine.search("test", top_k=10)
        assert len(result.results) == 10

    def test_vector_count_preserved_across_save(self):
        ...
        try:
            engine.save(path)
            engine2 = SemanticSearchEngine(FAISSVectorIndex(DIM), IndexMetadataStore(), encoder=None)
            engine2.load(path)
    ...
    def test_empty_index_search(self):
        engine = SemanticSearchEngine(FAISSVectorIndex(DIM), IndexMetadataStore(), encoder=dummy_encoder)
        result = engine.search("anything")
        assert result.total_count == 0
    ...
    def test_rebuild_replaces(self):
        ...
        stats = engine.build_index([{"repo_id": "new-a", "text": "new a", "metadata": {}}, {"repo_id": "new-b", "text": "new b", "metadata": {}}])
        assert stats.total_vectors == 2
        assert engine._metadata_store.repo_exists("old") is False
        assert engine._metadata_store.repo_exists("new-a")

    def test_batch_search_multiple(self):
        index = FAISSVectorIndex(DIM)
        store = IndexMetadataStore()
        engine = SemanticSearchEngine(index, store, encoder=dummy_encoder, batch_encoder=dummy_batch_encoder)
        engine.build_index([{"repo_id": f"r{i}", "text": f"content {i}", "metadata": {}} for i in range(10)])
...
class TestEdgeCases:
    def test_build_mismatched_lengths(self, faiss_index, metadata_store):
        engine = SemanticSearchEngine(faiss_index, metadata_store)
        with pytest.raises(ValueError):
            engine.build_index_from_embeddings(RNG.random((3, DIM)).astype(np.float32), ["only-one-id"])

    def test_add_embeddings_mismatched_lengths(self):
        se = SemanticSearchEngine(FAISSVectorIndex(DIM), IndexMetadataStore(), encoder=dummy_encoder)
        with pytest.raises(ValueError):
    ...
    def test_build_no_encoder_raises(self, faiss_index, metadata_store):
        engine = SemanticSearchEngine(faiss_index, metadata_store)
        with pytest.raises(ValueError):
            engine.build_index([{"repo_id": "x", "text": "y"}])

    def test_add_no_encoder_raises(self, faiss_index, metadata_store):
        engine = SemanticSearchEngine(faiss_index, metadata_store)
        with pytest.raises(ValueError):
