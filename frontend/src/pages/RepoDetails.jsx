...
export default function RepoDetails() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      try {
        const fetched = getGeneratedMockDetails(owner, repo);
        if (!fetched) {
  ...
  if (error || !details) {
    return (
      <div className="max-w-7xl mx-auto px-4 py-16 flex-1 flex flex-col justify-center items-center">
        <EmptyState type="error" title="Discovery Error" message={error || "Repository details could not be found."} onRetry={() => navigate(0)} />
      </div>
    );
  }
